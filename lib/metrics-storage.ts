...
export function storeMetric(entry: MetricEntry): void {
  try {
    ...
    console.warn(
      `Erreur lors du stockage de la métrique pour ${entry.appId}/${entry.templateId}/${entry.key}:`,
      error
    );
  }
}
 ...
 */
export function getMetrics(appId: string, templateId: string, key: string): MetricEntry[] {
  try {
    if (typeof window === "undefined" || !window.localStorage) {
      return [];
    }
    ...
    return metrics
      .filter(
        (entry) => entry && typeof entry.timestamp === "number" && typeof entry.value === "number"
      )
      .sort((a, b) => a.timestamp - b.timestamp);
  } catch (error) {
    console.warn(
      `Erreur lors de la récupération des métriques pour ${appId}/${templateId}/${key}:`,
      error
    );
    return [];
  }
...
export function clearMetricsForApp(appId: string): void {
  try {
    ...
    if (keysToRemove.length > 0) {
      console.log(
        `Suppression des métriques : ${keysToRemove.length} clé(s) supprimée(s) pour l'app ${appId}`
      );
    }
  } catch (error) {
