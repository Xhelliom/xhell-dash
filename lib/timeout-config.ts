 ...
 * ```
 */
export function getAdaptiveTimeout(templateId?: string, configuredTimeout?: number): number {
 ...
 * ```
 */
export function getTimeoutFromApp(app: {
  statsConfig?: { templateId?: string; timeout?: number };
}): number {
  return getAdaptiveTimeout(app.statsConfig?.templateId, app.statsConfig?.timeout);
}
