...
export function BackgroundConfigButton({
  ...
  const handleClick = async (e: React.MouseEvent) => {
    ...
    const intent = clickIntentRef.current ?? (isSaveMode && onSave ? "save" : "open");
  ...
  return (
    ...
    >
      <div
        className={cn("flex items-center", isHovered ? "justify-start" : "justify-center w-full")}
      >
        {isSaving ? (
        ...
        <span
          className={cn(
            ...
            isHovered ? `${maxTextWidth} opacity-100 ml-2 delay-150` : "max-w-0 opacity-0 ml-0"
          )}
        >
