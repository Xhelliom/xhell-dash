  ...
  const renderStructuredInfo = (info: InfoData) => (
    <div className="flex flex-col gap-0.5 min-w-0">
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground min-w-0">
        ...
        {info.date && <span className="shrink-0 text-primary/80">{formatInfoDate(info.date)}</span>}
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-1 min-w-0">
      ...
      {isLoading && !infoData ? (
        ...
        <span className="text-sm text-muted-foreground italic">Aucune information disponible</span>
      )}
