...
export function WidgetForm({ widget, onSubmit, onCancel }: WidgetFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {type === "clock" && (
        <ClockConfigForm config={config as ClockWidgetConfig} updateConfig={updateConfig} />
      )}

      {type === "weather" && (
        <WeatherConfigForm config={config as WeatherWidgetConfig} updateConfig={updateConfig} />
      )}
      ...
      <div className="flex gap-2 justify-end pt-4">
        ...
        <Button type="submit">{widget ? "Modifier" : "Ajouter"}</Button>
      </div>
    </form>
  );
}
