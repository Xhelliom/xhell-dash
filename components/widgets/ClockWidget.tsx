...
export function ClockWidget({ config }: ClockWidgetProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[150px] w-full">
      ...
      <div className="text-4xl font-bold mb-2 font-mono text-center">{formatTime()}</div>

      {/* Affichage de la date */}
      {showDate && <div className="text-sm text-muted-foreground text-center">{formatDate()}</div>}
    </div>
  );
}
