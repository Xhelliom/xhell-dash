 ...
 */
export function StyleSelector({ value, onValueChange, className }: StyleSelectorProps) {
  /**
   * Met à jour une propriété du preset de style
   */
  const updatePreset = <K extends keyof StylePreset>(key: K, newValue: StylePreset[K]) => {
    onValueChange({
      ...value,
      [key]: newValue,
    });
