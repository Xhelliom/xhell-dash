...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: "Impossible de récupérer les widgets" }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.type || !["clock", "weather", "system-info"].includes(body.type)) {
      return NextResponse.json({ error: "Type de widget invalide" }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: "Impossible de créer le widget" }, { status: 500 });
  }
}
