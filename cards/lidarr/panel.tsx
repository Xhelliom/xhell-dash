/**
 * Composant Panel pour les statistiques détaillées Lidarr
 *
...
import { Loader2, Music, Download, AlertCircle, CheckCircle2 } from "lucide-react";
...
/**
 * Composant du panneau de statistiques Lidarr
 *
 * Affiche les statistiques détaillées dans un Sheet (panneau latéral)
 */
export function LidarrStatsPanel({ open, onOpenChange, appId, appName }: StatsPanelProps) {
  const [stats, setStats] = useState<LidarrStats | null>(null);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (appResponse.ok) {
          ...
          if (app) {
            const templateId = app.statsConfig?.templateId || "lidarr";
        ...
        const statsResponse = await fetch(`/api/apps/${appId}/stats/lidarr`);
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="h-[90vh] max-h-[90vh] overflow-y-auto p-6">
        <SheetHeader className="pb-4 border-b px-0">
          <SheetTitle className="text-2xl">Statistiques {appName}</SheetTitle>
          <SheetDescription>
            Vue d'ensemble de vos artistes, albums et téléchargements Lidarr
          </SheetDescription>
        </SheetHeader>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <Card>
                    ...
                    <CardContent>
                      ...
                      <p className="text-xs text-muted-foreground">{stats.totalAlbums} albums</p>
                    </CardContent>
                  </Card>

                  <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                      <CardTitle className="text-sm font-medium">Albums Téléchargés</CardTitle>
                      <CheckCircle2 className="h-4 w-4 text-muted-foreground" />
                    </CardHeader>
                    <CardContent>
                      <div className="text-2xl font-bold">{stats.downloadedAlbums}</div>
                  ...
                  <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                      <CardTitle className="text-sm font-medium">Albums Manquants</CardTitle>
                      <AlertCircle className="h-4 w-4 text-muted-foreground" />
                    </CardHeader>
                    <CardContent>
                      <div className="text-2xl font-bold">{stats.missingAlbums}</div>
                  ...
                  <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                      <CardTitle className="text-sm font-medium">En téléchargement</CardTitle>
                      <Download className="h-4 w-4 text-muted-foreground" />
                    </CardHeader>
                    <CardContent>
                      <div className="text-2xl font-bold">{stats.queueDownloading}</div>
                      <p className="text-xs text-muted-foreground">
                        {stats.queuePending} en attente
                      </p>
                    </CardContent>
                  </Card>
                </div>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="text-center">
                      <div className="text-2xl font-bold">{stats.queueStats.total}</div>
