...
async function fetchKubernetesStats(
  ...
  let pendingPods = 0;
  let failedPods = 0;
  const namespaceStatsMap = new Map<
    string,
    { podCount: number; runningPods: number; failedPods: number }
  >();
  ...
  const namespaceStats: KubernetesNamespaceStat[] = Array.from(namespaceStatsMap.entries()).map(
    ([namespace, stats]) => ({
      namespace,
