...
async function fetchTrueNASStats(
      ...
      if (size === 0 && allocated === 0) {
        ...
        if (pool.size) {
          if (typeof pool.size === "object" && pool.size.raw !== undefined) {
            size =
              typeof pool.size.raw === "number" ? pool.size.raw : parseInt(pool.size.raw, 10) || 0;
        ...
        if (pool.allocated) {
          if (typeof pool.allocated === "object" && pool.allocated.raw !== undefined) {
            allocated =
              typeof pool.allocated.raw === "number"
                ? pool.allocated.raw
                : parseInt(pool.allocated.raw, 10) || 0;
  ...
  let cpuUsage = 0;
  const memoryUsage = 0;
  const memoryTotal = systemInfo.physmem || 0; // Mémoire physique totale depuis system/info
  const memoryUsed = 0;
