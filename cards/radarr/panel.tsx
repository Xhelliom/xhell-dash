/**
 * Composant Panel pour les statistiques détaillées Radarr
 *
...
/**
 * Composant du panneau de statistiques Radarr
 *
 * Affiche les statistiques détaillées dans un Sheet (panneau latéral)
 */
export function RadarrStatsPanel({ open, onOpenChange, appId, appName }: StatsPanelProps) {
  const [stats, setStats] = useState<RadarrStats | null>(null);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (appResponse.ok) {
          ...
          if (app) {
            const templateId = app.statsConfig?.templateId || "radarr";
        ...
        const statsResponse = await fetch(`/api/apps/${appId}/stats/radarr`);
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="h-[90vh] max-h-[90vh] overflow-y-auto p-6">
        <SheetHeader className="pb-4 border-b px-0">
          ...
          <SheetDescription>Vue d'ensemble de vos films et téléchargements Radarr</SheetDescription>
        </SheetHeader>
              ...
              {displayOptions?.showKPIs !== false && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <Card>
                    ...
                    <CardContent>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {stats.downloadedMovies} téléchargés
                      </p>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold">{stats.downloadedMovies}</div>
                      <p className="text-xs text-muted-foreground">Films disponibles</p>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold">{stats.missingMovies}</div>
