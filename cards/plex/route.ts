...
async function fetchPlexStats(baseUrl: string, token: string): Promise<PlexStats> {
  ...
  if (contentType.includes("application/json")) {
    ...
    try {
      ...
      throw new Error(
        "L'API Plex a retourné du XML au lieu de JSON. Veuillez vérifier la configuration."
      );
    }
  }
      ...
      if (libraryDetailsResponse.ok) {
        ...
        if (responseContentType.includes("application/json")) {
          ...
          try {
            ...
            console.warn(
              `Réponse non-JSON pour ${libraryName} (type: ${responseContentType}), utilisation de la valeur par défaut`
            );
        ...
        if (count === 0 && libraryType !== "photo") {
          console.log(
            `Bibliothèque ${libraryName} (${libraryType}): totalSize=${totalSize}, count=${count}`
          );
        }
        ...
        if (libraryType === "show") {
          try {
            ...
            if (episodesResponse.ok) {
              ...
              episodeCount =
                typeof episodesTotalSize === "string"
                  ? parseInt(episodesTotalSize, 10)
                  : episodesTotalSize || 0;
            }
          } catch (error) {
            console.warn(
              `Impossible de récupérer le nombre d'épisodes pour ${libraryName}:`,
              error
            );
          }
        }
    ...
    if (recentlyAddedResponse.ok) {
      ...
      for (const item of items) {
        ...
        recentMedia.push({
          title:
            item.title ||
            (item.grandparentTitle && item.grandparentTitle !== item.title
